    ...
    <nav class="nav">
      <menu class="nav__controls">
        ...
        <li
          ...
          v-for="(active, menu, i) in menus"
          :key="i"
          @click="setMenu(active, menu)"
        >
          {{ menu }}
        </li>
      ...
      <p class="nav__label" @click="modal = !modal">About This Pen</p>
    </nav>
    <transition-group
      name="dropdown"
      tag="section"
      ...
      >
        ...
        <ul class="company__details">
          <li class="company__data">
            ...
            <p class="company__country">{{ company.country }}</p>
          </li>
          <li class="company__data">
            <label class="company__label">Rating</label>
            <p class="company__rating">{{ company.rating }}</p>
          </li>
        </ul>
      </li>
    </transition-group>
      ...
      <section v-show="modal" class="modal" @click="modal = false">
        <article class="modal__content">
          ...
          <h5 class="modal__link">
            <a
              href="https://github.com/Ledvis/vue.js-transitions-animations"
              target="_blank"
            >
...
export default {
  ...
  computed: {
    activeMenu() {
      return Object.keys(this.menus).findIndex(menu => this.menus[menu]);
    },
    activeFilters() {
      const { countries, categories } = this.filters;

      return {
        countries: Object.keys(countries).filter(country => countries[country]),
    ...
    filteredCompanies() {
      const { countries, categories } = this.activeFilters;
      ...
      return this.companies.filter(({ country, keywords, rating }) => {
        ...
        if (
          categories.length &&
          !categories.every(category => keywords.includes(category))
        )
          return false;
        else return true;
      });
  ...
  watch: {
    activeMenu(indexTo, indexFrom) {
      if (indexTo === indexFrom) return;

      this.$nextTick(() => {
        if (!this.$refs.menu || !this.$refs.menu[indexTo]) {
          this.dropdown.height = 0;
        } else {
          this.dropdown.height =
            this.$refs.menu[indexTo].clientHeight + 16 + "px";
        }
      });
    }
  },
  methods: {
    setMenu(active, menu) {
      Object.keys(this.menus).forEach(tab => {
        this.menus[tab] = tab === menu && !active;
      });
    },
    setFilter(filter, option) {
      if (filter === "countries") {
        ...
        Object.keys(this.filters.categories).forEach(category => {
          this.filters[filter][category] =
            option === category && !this.filters[filter][option];
        });
      }
    },
    clearAllFilters() {
      Object.keys(this.filters).forEach(filter => {
        if (filter === "rating") this.filters.rating = this.rating.min;
        Object.keys(this.filters[filter]).forEach(
          option => (this.filters[filter][option] = false)
        );
      });
    }
  },
      ...
      .then(data => {
        ...
        data.forEach(({ country, keywords, rating }) => {
          ...
          keywords.forEach(keyword =>
            this.$set(this.filters.categories, keyword, false)
          );
        });
      });
  }
...
.filters {
  ...
  &__item {
    ...
    animation: stagger 1s cubic-bezier(0.19, 1, 0.22, 1) forwards;
    backface-visibility: hidden;
    cursor: pointer;
